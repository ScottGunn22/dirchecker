@echo off
:: Directory QC Script - Simplest Version with Clean Output
...
if exist "%ASP%" (
    call :getsize "%ASP%"
    if %SIZE% GTR 25600 (
        echo [OK] REQUESTINFO\%PREFIX%-Attack Surface Profile.xlsx - %SIZEKB% KB
    ) else (
        echo [FAIL] REQUESTINFO\%PREFIX%-Attack Surface Profile.xlsx - File too small: %SIZEKB% KB ^(needs ^> 25 KB^)
        set /a ERRORS+=1
    )
) else (
    echo [FAIL] Missing: REQUESTINFO\%PREFIX%-Attack Surface Profile.xlsx
    set /a ERRORS+=1
)
...
if /i "%TEST_TYPE%"=="SB" (
    ...
    if errorlevel 1 (
        echo [FAIL] Missing: NVA\NESSUS\*.nessus
        set /a ERRORS+=1
    ) else (
        echo [OK] NVA\NESSUS\*.nessus found
    )
    ...
    for %%T in (TCP UDP) do (
        for %%E in (gnmap nmap xml) do (
            if exist "%BASE_DIR%\NVA\NMAP\%PREFIX%_%%T.%%E" (
                echo [OK] NVA\NMAP\%PREFIX%_%%T.%%E
            ) else (
                echo [FAIL] Missing: NVA\NMAP\%PREFIX%_%%T.%%E
                set /a ERRORS+=1
            )
        )
    )
